...
export class ResumePageService {
	...
	public getResumePage(language: string): Observable<ResumePage> {
		...
		const personalInfoRef = resumePageRef.doc('personalInfo');

		const personalInfoFieldsRef = personalInfoRef.collection('fields').orderBy('order', 'asc');

		const headingRef = resumePageRef.doc('heading');
		...
		return from(
			Promise.all([personalInfoRef.get(), personalInfoFieldsRef.get(), headingRef.get(), historyRef.get()])
		).pipe(
			map((refs) => {
				return <ResumePage>{
					personalInfo: {
