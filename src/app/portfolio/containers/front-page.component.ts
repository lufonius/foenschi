...
export class FrontPageComponent {
	...
	constructor(public store: Store<any>, public scrollService: ScrollService) {
	...
	registerCurrentSectionOnChange() {
		this.store.pipe(select(fromPortfolio.getCurrentSectionState)).subscribe((currentSectionState) => {
			switch (currentSectionState) {
				case 'about-me': {
					this.goToAboutMeSection();
					break;
				}
				case 'contact': {
				...
				case 'projects': {
					this.goToProjectSection();
					break;
				}
				case 'entry': {
					this.goToEntrySection();
					break;
				}
			}
		});
	...
	registerQuickNavTitlesOnChange() {
		this.store
			.pipe(select(fromPortfolio.getQuickNavTitles))
		...
		this.moveToSection$
			.pipe(
				withLatestFrom(
					...
					(accessor, sectionPositions) => {
						return { sectionPositions, accessor };
					}
				),
